 ...
 * calls. This class sets up the MPI and APGAS runtime before launching the
 * Junit tests on every rank (or place in APGAS terminology). To use this class
 * over the default {@link MpiTestLauncher} to run your tests, you need to
 * specify this class in the {@link MpiConfig} annotation of your test class.
 *
 * @author Patrick Finnerty
 *
 */
final public class ParameterizedMpiApgasTestLauncher implements Launcher {
	...
	/**
	 * Main method of the {@link ParameterizedMpiApgasTestLauncher} class.
	 * <p>
	 * Sets up the APGAS environment using MPI. Rank 0 of the processes will launch
	 * the main method of the class specified as parameter with the arguments
	 * specified afterward.
	 * <p>
	 ...
	 *             tests are to launch, the second argument is the path under which
	 *             the {@link Notification}s sent to Junit are to be written.
	 * @throws Throwable if thrown as part of the main method
	 */
	public static void main(String[] args) throws Throwable {
		...
		// method is launched
		System.setProperty(Config.APGAS_LAUNCHER, ParameterizedMpiApgasTestLauncher.class.getCanonicalName());

		/*
		 * If this place is the "master", i.e. rank 0, indireclty broadcasts connection
		 * information to every other hosts by setting up the runtime with this
		 * launcher. If this is not the master, receives that information and uses it to
		 * setup the runtime.
		 */
		if (commRank == 0) {
			...
			final String notificationFileName = testClass.getCanonicalName() + "_" + configurationIndex + "_"
					+ commRank;
	...
	/**
	 * Constructs a new {@link ParameterizedMpiApgasTestLauncher} instance.
	 */
	public ParameterizedMpiApgasTestLauncher() {
	}

	/**
	 * Checks that all the processes launched are healthy.
	 * <p>
	 * Current implementation of {@link ParameterizedMpiApgasTestLauncher} always
	 * returns true.
	 *
	 ...
	 * repeated.
	 * <p>
	 * In this particular launcher implementation, this method is used by rank 0 to
	 * broadcast setup information to every other process which are expecting it in
	 * the {@link #slave()} method.
	 *
